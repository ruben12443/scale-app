...
func (r *userRepo) Create(ctx context.Context, u *domain.User) error {
	...
	_, err := s.pool.Exec(ctx,
		`INSERT INTO users (id, tenant_id, rauthy_subject_id, display_name, email, role, created_at)
		 VALUES ($1, $2, $3, $4, $5, $6, $7)`,
		u.ID, u.TenantID, u.RauthySubjectID, u.DisplayName, u.Email, u.Role, u.CreatedAt)
	if err != nil {
...
func (r *userRepo) Get(ctx context.Context, id string) (*domain.User, error) {
	...
	row := s.pool.QueryRow(ctx,
		`SELECT id, tenant_id, rauthy_subject_id, display_name, email, role, created_at
...
func (r *userRepo) GetByRauthySubject(ctx context.Context, subject string) (*domain.User, error) {
	...
	row := s.pool.QueryRow(ctx,
		`SELECT id, tenant_id, rauthy_subject_id, display_name, email, role, created_at
		 FROM users WHERE rauthy_subject_id = $1`, subject)
	return scanUser(row)
}
...
func (r *userRepo) ListByTenant(ctx context.Context, tenantID string) ([]*domain.User, error) {
	...
	for rows.Next() {
		var u domain.User
		if err := rows.Scan(&u.ID, &u.TenantID, &u.RauthySubjectID, &u.DisplayName, &u.Email, &u.Role, &u.CreatedAt); err != nil {
...
func scanUser(row rowScanner) (*domain.User, error) {
	var u domain.User
	err := row.Scan(&u.ID, &u.TenantID, &u.RauthySubjectID, &u.DisplayName, &u.Email, &u.Role, &u.CreatedAt)
	if err != nil {
