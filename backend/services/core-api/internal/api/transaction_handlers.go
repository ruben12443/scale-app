...
func (h *TransactionHandlers) Create(w http.ResponseWriter, r *http.Request) {
	...
	if req.ProductID == "" || req.ScaleID == "" {
		writeError(w, http.StatusBadRequest, "product_id and scale_id are required")
		return
	}
	if req.WeightGrams < 0 || req.UnitPriceCents < 0 || req.TotalPriceCents < 0 {
		writeError(w, http.StatusBadRequest, "weight_grams, unit_price_cents, and total_price_cents must not be negative")
		return
	}
