...
- `docker-compose.yml` — local dev stack: Postgres (core-api's own
  database), Rauthy (a single self-contained OIDC provider — no separate
  database or login-UI service, unlike Zitadel, which this replaced),
  Mailpit (a local SMTP catcher with a web UI so receipt emails are visible
  during dev without a real mail account), scale-gateway, core-api, and
  `mobile-web` (serves the vendor app as an installable PWA).
- `rauthy-bootstrap/` — JSON files Rauthy reads once, on its very first
  start against an empty database, to provision the mobile app's OIDC
  client and the API key core-api uses to manage vendor identities — see
  `rauthy-bootstrap/README.md`.
...
`POSTGRES_PASSWORD`, `POSTGRES_DB`, `RAUTHY_DOMAIN`, `RAUTHY_ADMIN_PASSWORD`,
`RAUTHY_ENC_KEYS`, `RAUTHY_ENC_KEY_ACTIVE`, `RAUTHY_HQL_SECRET`,
`RAUTHY_API_KEY_SECRET`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`,
`CURRENCY`), then:
...
`RAUTHY_DOMAIN` is the host the mobile app/browser will reach this stack
...
a phone" below. It's baked into Rauthy's first-run bootstrap data, so
changing it requires `docker compose down -v && docker compose up --build`
to take effect, same as before — but unlike the previous Zitadel setup,
there's no manual console step: Rauthy's OIDC client (the mobile app) and
its admin API key are both provisioned declaratively from
`rauthy-bootstrap/*.json` on that same first start. Testing from a new
origin (a LAN IP, or a different port) does need that origin added to
`rauthy-bootstrap/clients.json`'s `redirect_uris` first — see
`rauthy-bootstrap/README.md` for why a wildcard can't cover this. Stripe
...
from the services' own verified configuration and Rauthy's own documented
compose/bootstrap patterns (fetched from its live docs and OpenAPI spec
while building this, not guessed), but treat it as unverified until run for
real. Same caveat applies to the Rauthy and Stripe API integrations
themselves; see `backend/services/core-api/README.md` for specifics.
...
2. The native app's redirect scheme is host-independent, so it needs no
   change — but if you'll *also* test the installed PWA from that phone,
   add `http://192.168.1.42:8083/redirect.html` to
   `rauthy-bootstrap/clients.json`'s `redirect_uris` first.
3. Set `RAUTHY_DOMAIN=192.168.1.42` in `.env`, then
   `docker compose down -v && docker compose up --build` (a fresh Rauthy
   init is required — the domain is baked into its first-run bootstrap
   data).
4. Run the app pointed at the same IP for every backend it talks to:
   ```
   flutter run \
     --dart-define=CORE_API_BASE_URL=http://192.168.1.42:8081 \
     --dart-define=SCALE_GATEWAY_BASE_URL=http://192.168.1.42:8082 \
     --dart-define=RAUTHY_ISSUER=http://192.168.1.42:8080/auth/v1 \
     --dart-define=RAUTHY_CLIENT_ID=stallhand
   ```

All the values above must agree with each other and with `RAUTHY_DOMAIN` —
mixing `localhost` and a LAN IP across them breaks OIDC discovery (the
issuer Rauthy advertises must match the URL the phone actually reached it
at) and redirect-URI matching. Switch back to `localhost` the same way
when done.
